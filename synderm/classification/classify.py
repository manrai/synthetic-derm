...
class Args(Tap):
    image_dir: str = "/n/data1/hms/dbmi/manrai/derm/"
    wandb_project: str = "dose_response_june7"
    random_state: int = 119108
    generation_folder: str = "generations-more/text-to-image/"
    n_per_label_list: list[int] = [128, 228]
    include_synthetic_list: list[bool] = [True]
    n_synthetic_per_real_list: list[int] = [10, 25, 50, 75]
    generation_type_list: list[str] = ["text-to-image"]
...
def generate_synthetic_metadata(
    ...
    """
    df: pd.DataFrame = pd.concat([train_val_df] * n_synthetic_per_real, ignore_index=True)

    # create a variable that represents the nth copy of the image
    df["n"] = df.groupby("md5hash").cumcount() + 10
    df["location"] = (
        folder
        + df["label"].str.replace(" ", "-")
        + "/"
        + generation_type
        + "/"
        + df["n"].astype(str)
...
def get_n_per_label_data(
    ...
    real_data: pd.DataFrame = train_data[~train_data["synthetic"]]
    n_per_label_data: pd.DataFrame = pd.DataFrame(
        real_data.groupby(["label"])
        .apply(lambda x: x.sample(n=n_per_label, random_state=random_state))
        .reset_index(drop=True)
    )

    if include_synthetic:
        # Get the md5hashes of the real images selected
        md5hashes: pd.Series = n_per_label_data["md5hash"]
        # Get the synthetic images that share the same md5hash as the real images selected
        synthetic_data: pd.DataFrame = train_data[train_data["md5hash"].isin(md5hashes)]
        # Concatenate the n_per_label data with the val data
        n_per_label_data = pd.concat([n_per_label_data, synthetic_data])
    else:
        # Concatenate the n_per_label data with the val data
...
def main():
    ...
    test_data["location"] = (
        "Fitzpatrick17k/finalfitz17k/" + test_data["md5hash"] + ".jpg"
    )
    ...
    for combo in itertools.product(
        n_per_label_list,
        include_synthetic_list,
        n_synthetic_per_real_list,
        ...
        train_val_data = pd.DataFrame(
            metadata[~metadata["md5hash"].isin(test_data["md5hash"])]
        )
        # Get the synthetic metadata
        synthetic_metadata = generate_synthetic_metadata(
            train_val_data,
        ...
        train_val_data = pd.concat([train_val_data, synthetic_metadata]).reset_index(
            drop=True
        )
        ...
        val_data = pd.DataFrame(
            real_train_val_data.groupby(["label"])
            .apply(lambda x: x.sample(n=40, random_state=random_state, replace=False))
            .reset_index(drop=True)
        )
        train_data = pd.DataFrame(
            train_val_data[~train_val_data["md5hash"].isin(val_data["md5hash"])]
        )
        ...
        # Get the data
        df = get_n_per_label_data(
            train_data, val_data, n_per_label, random_state, include_synthetic
        )

        # Print a summary of the run data
        print(
            "Number of real training images per label:",
            len(df[(df["synthetic"] == False) & (df["is_valid"] == False)])
            / df["label"].nunique(),
        )
        print(
            "Number of synthetic training images per label:",
            len(df[(df["synthetic"] == True) & (df["is_valid"] == False)])
            / df["label"].nunique(),
        )
        ...
        learn = vision_learner(
            dls, arch=efficientnet_v2_m, metrics=[error_rate, accuracy]
        )
        ...
        top1_ci_lower, top1_ci_upper = proportion_confint(
            top1_acc * len(true_labels), len(true_labels), alpha=0.05, method="normal"
        )
        top3_ci_lower, top3_ci_upper = proportion_confint(
            top3_acc * len(true_labels), len(true_labels), alpha=0.05, method="normal"
        )
...
if __name__ == "__main__":
    ...
    n_per_label_list = args.n_per_label_list
    include_synthetic_list = args.include_synthetic_list
    n_synthetic_per_real_list = args.n_synthetic_per_real_list
    generation_type_list = args.generation_type_list

    # Create a dictionary to store the different types of augmentation
    augmentation_dict = {"None": [], "aug_transforms": aug_transforms()}
