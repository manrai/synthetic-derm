...
class FitzpatrickDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        if self.dataset_type == "fitzpatrick":
            ...
            if disease_class in CLASS_NAMES:
                csv_file = (
                    ...
                    / "fitzpatrick17k_10label_clean_training.csv"
                )
            else:
            ...
            if self.data_disease_class != "all":
                self.data_df = self.data_df[
                    self.data_df["label"] == self.data_disease_class
                ]
        ...
        self.image_transforms = transforms.Compose(
            image_transforms + normalize_and_to_tensor
        )
    ...
    def __getitem__(self, index):
        ...
        filename = (
            entry.DDI_file if hasattr(entry, "DDI_file") else f"{entry.md5hash}.jpg"
        )
        example["image_name"] = filename.split(".")[0]
        instance_image_path = self.data_images_root / filename
        instance_class_name = (
            entry.disease.replace("-", " ")
            if hasattr(entry, "disease")
            else entry.label
        )
        ...
        if self.add_fitzpatrick_scale_to_prompt:
            fitzpatrick_scale = (
                str(entry.fitzpatrick_scale)
                if entry.fitzpatrick_scale > 0
                else "unknown"
            )
